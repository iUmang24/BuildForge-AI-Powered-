...
export default function EmailTemplatesPage() {
  ...
  const fetchTemplates = async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/admin/email-templates?search=${search}&page=${page}&limit=${limit}`,
        {
      ...
      setTemplates(json.data?.rows || json.data || []);
    } catch (err) {
  ...
  const handleCreate = async () => {
    if (!newTemplate.template_key || !newTemplate.subject) {
      return alert("Template key & subject required");
    }

    await fetch(`${API_BASE_URL}/admin/email-templates`, {
      method: "POST",
      headers: {
    ...
    setShowModal(false);
    setNewTemplate({ template_key: "", subject: "", description: "" });
    fetchTemplates();
  };
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-card border rounded-lg w-[500px] p-6 space-y-4">
            ...
            <input
              placeholder="Template Key (e.g. final_approved)"
